...
SELECT a.id, a.first_name, a.last_name, role.title AS role, department.name AS department, role.salary AS salary, CONCAT_WS(' ', b.first_name, b.last_name) AS manager
FROM employee a
...
const employeeInsert = (({ first, last, role, manager, confirmManager }) => {
    ...
    const sql = `INSERT INTO  (first_name, last_name, role_id, manager_id) VALUES (?, ?, (SELECT id FROM role WHERE title = ?), ?)`;
...
const employeeUpdateReturn = (({ newRole, employee, confirmUpdateManager, newManager }) => {
    ...
    const sql = `UPDATE employee SET role_id = (SELECT id FROM role WHERE title = ?), manager_id = ? WHERE id = ?`;
...
employeeDestroyInsert = (({ destroyEmployee }) => {
    const sql = `DELETE FROM employee WHERE CONCAT_WS(' ', first_name, last_name) = ?`;
